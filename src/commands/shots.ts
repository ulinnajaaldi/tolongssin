import { confirm, isCancel, cancel, text } from '@clack/prompts'
import { analyzeRepo } from '../lib/analyzer.js'
import { loadConfig } from '../lib/config.js'
import { startDevServer } from '../lib/server.js'
import { buildCapturePlan, DEFAULT_PORT, sectionFileName } from '../lib/shots-options.js'
...
export async function runShots(opts: { dryRun?: boolean }): Promise<void> {
  ...
  if (!plan.url) {
    const url = await text({ message: 'Enter the running app URL', placeholder: 'http://localhost:5173' })
    if (isCancel(url)) { cancel('Cancelled'); return }
    plan.url = url
  }
  ...
  if (opts.dryRun) {
    ...
    for (const [i, sel] of plan.sections.entries()) console.log(`  ${sectionFileName(i + 1)} (selector "${sel}")`)
  ...
  try {
    const needsAuth = await confirm({ message: 'Does this app require login?' })
    if (isCancel(needsAuth)) { cancel('Cancelled'); return }
    const results = await runCaptures(plan, process.cwd(), needsAuth === true)
    console.log('\nCaptured:')
    for (const r of results) console.log(`  ${r.path} (${r.width}x${r.height})`)
  } finally {
    server?.kill()
  }
}
