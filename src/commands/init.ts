...
export async function runInit(opts: { dryRun?: boolean }): Promise<void> {
  ...
  console.log(
    `Detected repo type: ${info.repoType} (${info.confidence} confidence)`,
  );
  ...
  if (apiKey) {
    ...
    s.stop(
      suggestions ? "Suggestions ready." : "No suggestions — fill manually.",
    );
  }
  ...
  console.log(
    "Added .tolongssin/.env and marketing-kit/ to .gitignore (if missing)",
  );
}
