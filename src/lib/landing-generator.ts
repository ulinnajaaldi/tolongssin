...
export const SectionPlanSchema = z.object({
  ...
  accentHue: z
    .string()
    .describe(
      "Single accent color as CSS hex, e.g. #E15554 — one color used sparingly, no gradients",
    ),
  displayFont: z
    .string()
    .describe("Google Font name for headings, never Inter"),
  bodyFont: z
    .string()
    .describe("Google Font name for body text, distinct from displayFont"),
});
...
export function buildLandingPrompt(input: LandingPromptInput): string {
  const historyNote =
    input.recentMacrostructures.length > 0
...
export async function fetchStudySignals(url: string): Promise<string | null> {
  try {
    ...
    for (const m of fontMatches)
      signals.push(`Fonts used: ${decodeURIComponent(m[1])}`);
    if (/[class*="hero" class*="banner"]/.test(html))
      signals.push("Layout hint: has hero/banner section");
    if (/grid|two-col|split/i.test(html))
      signals.push("Layout hint: grid or split layout detected");
...
export function slopTest(html: string): string[] {
  ...
  if (/Inter/gi.test(html) && (html.match(/font-family/gi)?.length ?? 0) > 0) {
    ...
    if (
      displayMatch &&
      bodyMatch &&
      /Inter/i.test(displayMatch) &&
      /Inter/i.test(bodyMatch)
    ) {
  ...
  if (
    /\bfont-family\s*:\s*(?:'[^']*'|"[^"]*"|\w+)\s*,\s*(?:Roboto|Open\s*Sans|Poppins|Lato|Arial|Helvetica)\b/i.test(
      html,
    ) === false &&
    /\bfont-family\s*:\s*(?:Roboto|Open\s*Sans|Poppins|Lato|Arial|Helvetica|sans-serif|system-ui|ui-sans-serif)\b/i.test(
      html,
    )
  ) {
    violations.push(
      "Default/system font detected (Roboto, Open Sans, Poppins, Lato, Arial, etc.)",
    );
  }
  // Italic headers
  ...
  if (
    /linear-gradient\([^)]*(?:purple|#8b5cf6|#a855f7|#ec4899|#d946ef|#f472b6|#6366f1|#8b5cf6|#06b6d4|#ec4899)/i.test(
      html,
    )
  ) {
  ...
  if (
    /background-clip\s*:\s*text/i.test(html) &&
  ...
  if (
    /grid-template-columns\s*:\s*repeat\s*\(\s*3\s*,\s*1fr\s*\)/i.test(html) &&
    /icon|svg/i.test(html)
  ) {
    violations.push("3-equal-column icon-above-heading card grid detected");
  }
  if (
    /min-height\s*:\s*100vh/i.test(html) &&
  ...
  if (
    /traffic|dots?\s*(?:red|yellow|green)|#ff5f56|#ffbd2e|#27c93f/.test(html)
  ) {
    violations.push("Fake browser chrome (traffic-light dots) detected");
  }
  if (
    /\bframe\b[^>]*>\s*<img|border-radius\s*:\s*3rem[^;]*;\s*[^}]*padding\s*:\s*1rem/i.test(
      html,
    )
  ) {
  ...
  // Emoji overload
  const emojiCount = (
    html.match(/[\p{Emoji_Presentation}\p{Extended_Pictographic}]/gu) ?? []
  ).length;
  if (emojiCount > 10) violations.push("Emoji overload detected");

  // Invented metrics / testimonials
  if (
    /\+\d+\s*%|\d+\s*\+?\s*(?:users|teams|customers|stars|downloads)|trusted by|10x|50,000|100k/i.test(
      html,
    )
  ) {
  ...
  // Glassmorphism
  if (
    /backdrop-filter\s*:\s*blur|rgba\(\s*255,\s*255,\s*255,\s*0\.[0-9]+\s*\)/.test(
      html,
    )
  ) {
    violations.push(
      "Glassmorphism (backdrop blur / translucent white) detected",
    );
  }
...
export async function generateLanding(
  ...
  if (opts.study) {
    ...
    if (!fetched)
      console.warn(
        `Could not fetch ${opts.study} — proceeding without study signals.`,
      );
  }

  const repoContext = repoInfo
    ? [
        repoInfo.dependencies?.length
          ? `Dependencies: ${repoInfo.dependencies.join(", ")}`
          : null,
        repoInfo.fileStructure?.length
          ? `Top-level structure: ${repoInfo.fileStructure.join(", ")}`
          : null,
      ]
        .filter(Boolean)
        .join("\n")
  ...
  if (opts.dryRun) {
    ...
    console.log(
      JSON.stringify(
        violations.length > 0 ? { ...plan, slopViolations: violations } : plan,
        null,
        2,
      ),
    );
  ...
  cfg.landingPage = {
    ...
    macrostructureHistory: updatedHistory,
  };

  const landingDir = join(cwd, "marketing-kit", "landing");
  mkdirSync(landingDir, { recursive: true });
  writeFileSync(
    join(landingDir, violations.length > 0 ? "draft.html" : "index.html"),
    html,
    "utf8",
  );
  writeFileSync(
    join(landingDir, "README.md"),
  ...
  if (existsSync(configPath)) {
    const existing: TolongssinConfig = JSON.parse(
