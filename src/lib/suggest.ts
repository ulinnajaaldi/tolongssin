...
const SuggestionSchema = z.object({
  projectName: z.string().describe('The project name, derived from package.json name or repo name'),
  valueProp: z.string().describe('A concise one-line value proposition (max 80 chars)'),
  audience: z.string().describe('Target audience (e.g. "Developers shipping side projects", "DevOps engineers")'),
  primaryCta: z.string().describe('A short call-to-action button text (e.g. "Get started", "Try it", "Star on GitHub")'),
})

export type Suggestions = z.infer<typeof SuggestionSchema>

export async function suggestConfig(info: RepoInfo): Promise<Suggestions | null> {
  const meta = [
    ...
    info.dependencies?.length ? `Dependencies: ${info.dependencies.join(', ')}` : null,
    info.fileStructure?.length ? `Top-level files: ${info.fileStructure.join(', ')}` : null,
  ].filter(Boolean).join('\n')
