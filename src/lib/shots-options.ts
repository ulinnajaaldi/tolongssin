...
export function resolveTargetUrl(configScreenshotsUrl: string | undefined, serverCommand: string | null, port: number): string {
...
export function buildCapturePlan(
  ...
  if (!isWebApp) {
    return {
      plan: { desktopFull: false, mobileTop: false, sections: [], url: '', serverCommand: null },
      skipReason: 'This project does not look like a web app — skipping screenshots.\n(README + social drafts still work; try `tolongssin readme`.)',
    }
  }
  return {
    plan: {
