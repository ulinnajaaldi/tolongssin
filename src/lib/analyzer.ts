...
export function analyzeRepo(cwd: string): RepoInfo {
  ...
  const isWebApp =
    hasWebFrameworkDep || (hasReact && hasReactDom) || hasPyWeb || hasIndexHtml;
  ...
  const repoType: RepoType =
    trueSignals.length > 0 ? trueSignals[0][0] : "other";
  const confidence: RepoInfo["confidence"] =
    trueSignals.length === 1
      ? "high"
      : trueSignals.length > 1
        ? "medium"
        : "low";
  ...
  const hasDevScript =
    typeof scripts === "object" && scripts !== null
      ? (["dev", "start", "preview"] as const).find(
          (script) =>
