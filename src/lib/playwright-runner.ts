...
export async function runCaptures(plan: CapturePlan, cwd: string, headed = false): Promise<CaptureResult[]> {
  ...
  try {
    ...
    if (plan.mobileTop) {
      ...
      await mobile.goto(plan.url, { waitUntil: 'networkidle', timeout: 30_000 })
      await mobile.waitForTimeout(500)
      const path = join(outDir, 'mobile-top.png')
      await mobile.screenshot({ path, fullPage: false })
      const dims = await mobile.evaluate(() => ({ w: window.innerWidth, h: window.innerHeight }))
      results.push({ path, width: dims.w, height: dims.h })
      await mobile.close()
    }
