...
export async function askProjectInfo(
  ...
  const projectName = await text({
    ...
    initialValue: pref.projectName,
  });
  if (isCancel(projectName)) return undefined;
  const valueProp = await text({
    message: "One-line value proposition",
    initialValue: pref.valueProp,
  });
  if (isCancel(valueProp)) return undefined;
  const audience = await text({
    message: "Target audience",
    initialValue: pref.audience,
  });
  ...
  const primaryCta = await text({
    message: "Primary CTA (optional, for landing)",
    initialValue: pref.primaryCta,
