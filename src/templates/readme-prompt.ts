...
export function buildReadmePrompt(input: ReadmePromptInput): string {
  const meta = [
    ...
    input.repoInfo.description ? `Description: ${input.repoInfo.description}` : null,
    input.repoInfo.keywords?.length ? `Keywords: ${input.repoInfo.keywords.join(', ')}` : null,
    input.repoInfo.repoUrl ? `Repo URL: ${input.repoInfo.repoUrl}` : null,
    input.repoInfo.dependencies?.length ? `Dependencies: ${input.repoInfo.dependencies.join(', ')}` : null,
    input.repoInfo.fileStructure?.length ? `Top-level structure: ${input.repoInfo.fileStructure.join(', ')}` : null,
  ].filter(Boolean).join('\n')
