...
import {
  ...
  generateReadme,
  scrubSecrets,
} from "../src/lib/readme-generator.js";
...
describe("generateReadme", () => {
  ...
  it("confirm yes -> write + backup", async () => {
    ...
    expect(
      readFileSync(join(dir, ".tolongssin", "backups", backups[0]), "utf8"),
    ).toBe("old readme");
  });
  ...
  it("confirm no -> draft only", async () => {
    ...
    expect(
      readFileSync(join(dir, "marketing-kit", "README.draft.md"), "utf8"),
    ).toBe("# Demo\n\nHook line\n");
  });
