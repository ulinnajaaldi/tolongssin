...
import { buildCapturePlan, resolveTargetUrl, sectionFileName, DEFAULT_PORT } from '../src/lib/shots-options.js'

describe('buildCapturePlan', () => {
  it('skips non-web projects', () => {
    const { plan, skipReason } = buildCapturePlan({}, false, undefined, null, undefined)
  ...
  it('prefers the configured url', () => {
    const { plan } = buildCapturePlan({}, true, 'https://example.com', 'dev', undefined)
    expect(plan.url).toBe('https://example.com')
  })

  it('maps configured sections', () => {
    const { plan } = buildCapturePlan({}, true, undefined, null, ['#hero', '#features'])
...
describe('resolveTargetUrl', () => {
  it('returns the config url when set', () => {
    expect(resolveTargetUrl('https://example.com', 'dev', 5173)).toBe('https://example.com')
  })

  it('falls back to localhost for a dev script', () => {
    expect(resolveTargetUrl(undefined, 'dev', 5173)).toBe('http://localhost:5173')
  })
