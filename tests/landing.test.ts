...
import {
  ...
  SectionPlanSchema,
  slopTest,
} from "../src/lib/landing-generator.js";
...
describe("buildLandingPrompt", () => {
  ...
  it("excludes recent macrostructures", () => {
    ...
    expect(prompt).toContain(
      "Do NOT use any of these recently used macrostructures: split-hero",
    );
  });
...
describe("SectionPlanSchema", () => {
  ...
  it("rejects missing macrostructure", () => {
    expect(() =>
      SectionPlanSchema.parse({ ...samplePlan, macrostructure: "invalid" }),
    ).toThrow();
  });
});
...
describe("slopTest", () => {
  it("catches Inter for both fonts", () => {
    const html =
      "<html><style>:root{--display-font: Inter; --body-font: Inter; font-family: Inter;}</style><body>hi</body></html>";
  ...
  it("catches purple gradient", () => {
    const html =
      '<div style="background: linear-gradient(purple, pink)"></div>';
  ...
  it("catches gradient text (background-clip: text)", () => {
    const html =
      '<h1 style="background: linear-gradient(90deg, #f00, #00f); -webkit-background-clip: text; background-clip: text; color: transparent">Gradient</h1>';
    const v = slopTest(html);
  ...
  it("catches fake browser chrome (traffic-light dots)", () => {
    const html =
      '<div class="browser-bar"><span style="background:#ff5f56"></span><span style="background:#ffbd2e"></span><span style="background:#27c93f"></span></div>';
    const v = slopTest(html);
  ...
  it("catches glassmorphism (backdrop blur)", () => {
    const html =
      '<div style="backdrop-filter: blur(8px); background: rgba(255,255,255,0.8)">glass</div>';
    const v = slopTest(html);
