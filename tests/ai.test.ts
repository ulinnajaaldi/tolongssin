...
describe("generateMarkdown", () => {
  it("returns generated markdown from generateText", async () => {
    ...
    await expect(generateMarkdown("write hello")).resolves.toBe(
      "## Hello\n\nA CLI tool that generates marketing kits from your repo.",
    );
    expect(mocks.generateText).toHaveBeenCalledWith(
      expect.objectContaining({
        prompt: expect.stringContaining("write hello"),
  ...
  it("maps 401 errors to a friendly AiError and never leaks the key", async () => {
    mocks.generateText.mockRejectedValue(
      new Error("401 invalid api key sk-test-123"),
    );
  ...
  it("falls back to a working model when configured model returns planning text", async () => {
    mocks.generateText
      .mockResolvedValueOnce({
        text: "Let me check the working directory first.",
      }) // configured model → planning
  ...
  it("throws friendly error when no fallback model works", async () => {
    ...
    await expect(generateMarkdown("write readme")).rejects.toThrow(
      "no fallback model worked",
    );
    vi.unstubAllGlobals();
  });
...
describe("generateJSON", () => {
  it("returns the typed object from generateObject", async () => {
    ...
    await expect(generateJSON("parse me", schema)).resolves.toEqual({
      ok: true,
    });
    expect(mocks.generateObject).toHaveBeenCalledWith(
      expect.objectContaining({
        prompt: expect.stringContaining("parse me"),
        schema,
      }),
    );
  });
...
describe("textOrReasoning", () => {
  ...
  it("rejects planning prose with no markdown structure", () => {
    const planning =
      "I'll quickly verify the actual CLI commands and structure before rewriting the README, so the quickstart is accurate.";
  ...
  it("strips tool-call blocks", () => {
    ...
    expect(textOrReasoning({ text: dirty })).toBe(
      "# Real README\n\nBody here.",
    );
  });
  ...
  it("reads AI SDK v7 `reasoning` field (not just reasoning_content)", () => {
    ...
    expect(
      textOrReasoning({ text: "planning only", reasoning: reasoning }),
    ).toBe(reasoning);
  });
});
...
describe("sanitizeModelText", () => {
  it("keeps JSON output (generateJSON flow)", () => {
    const json =
      '{"projectName":"qcli","valueProp":"Zero-config scaffolding","audience":"Developers","primaryCta":"npx qcli init"}';
    expect(sanitizeModelText(json)).toBe(json);
  });

  it("keeps X thread format (1/5 numbering)", () => {
    const thread =
      "Screenshot Hook: Terminal output.\n\n1/5\nWhy is verifying still a manual headache?\n\nIntroducing `test-proj`. 🧵\n\n2/5\nWe got tired of scratch scripts.";
    expect(sanitizeModelText(thread)).toBe(thread);
  });

  it("keeps LinkedIn multi-paragraph prose", () => {
    const post =
      "We built test-proj because verifying integrations was a pain.\n\nNow it is a 10-second check.\n\n#devtools #testing";
    expect(sanitizeModelText(post)).toBe(post);
  });

  it("keeps single-paragraph prose (README descriptions)", () => {
    const desc =
      "**test-proj** is a command-line tool for scaffolding, managing, and testing project templates directly from your terminal. It streamlines common project setup tasks so you can spin up or validate a project structure with a single command. Designed for developers who want speed without sacrificing reproducibility.";
  ...
  it("rejects tool-call planning prose", () => {
    const planning =
      'I\'ll take a quick look at the repo to ground the post.\n\nLet me check the repository contents and README.\n\n<tool_calls>\n<invoke name="read_file"><parameter name="file">/app/package.json</parameter></invoke>\n</tool_calls>';
    expect(sanitizeModelText(planning)).toBe("");
  });
});
